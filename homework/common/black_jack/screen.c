     ...
     lcd_draw_image(
        ...
        for(int i = 0; str[i] != NULL; i++){
            ...
            font_get_image(str[i], &C);
             ...
             lcd_draw_rectangle(
                LOC_GAME_STATS_FUNDS_X - 4 +(i*20), // x start
                C.width + 8,
                LOC_GAME_STATS_FUNDS_Y - 10 - 4, // y start
                C.height + 8,
                LCD_COLOR_BLACK,
                false);
    ...
    font_get_image(':', &colon);
     ...
     lcd_draw_image(
        ...
        for(int i = 0; str[i] != NULL; i++){
            ...
            lcd_draw_rectangle(
                LOC_GAME_STATS_BET_X - 4 +(i*20), // x start
                C.width + 8,
                LOC_GAME_STATS_BET_Y - 10 - 4, // y start
                C.height + 8,
                LCD_COLOR_BLACK,
                false);
...
void screen_display_stats_dealer_hand(int16_t hand_value, uint16_t fcolor) 
{
     ...
     for(int i = 0; str[i] != NULL; i++){
         ...
         lcd_draw_image(
            ...
            LOC_GAME_STATS_DEALER_HAND_Y - 10,
             C.width,
             C.height,
             C.bitmap,
             fcolor,
...
void screen_display_stats_player_hand(int16_t hand_value, uint16_t fcolor) 
{
    ...
    for(int i = 0; str[i] != NULL; i++){
        ...
        lcd_draw_image(
           ...
           LOC_GAME_STATS_PLAYER_HAND_Y - 10,
            C.width,
            C.height,
            C.bitmap,
            fcolor,
